 ...
 */
	...
	foreach ($ses_wpscd_result_rows as $ses_wpscd_row) {
		if ($ses_wpscd_row['status'] == 'Complete') {
			$ses_wpscd_date_range_completed[$ses_wpscd_row['textcdate']]['value'] = $ses_wpscd_row['totalprice'];
		} else {
			$ses_wpscd_date_range_pending[$ses_wpscd_row['textcdate']]['value'] = $ses_wpscd_row['totalprice'];
		}
	}

 	// Dataset definition      
 	$ses_wpscd_dataset = new pData;     
	$ses_wpscd_dataset->SetSerieName("Revenue","Serie1");  
	...
	if (count($ses_wpscd_date_range_pending)) {
		while (list($ses_wpscd_date,$ses_wpscd_items) = each ($ses_wpscd_date_range_pending)) {
			$ses_wpscd_dataset->AddPoint($ses_wpscd_items['value'],"Serie2");
		}
	}
